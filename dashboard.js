...
async function renderDashboard() {
  ...
  if (_dashCurrentDept === 'FnB') {
    shifts = allShifts.filter(function(s) { return s.area === 'Cocina' || s.area === 'Sala' || s.area === 'Friegue'; });
    mermas = allMermas.filter(function(m) { return m.area === 'Cocina' || !m.area; });
    incis = allIncis.filter(function(i) { return i.area === 'Cocina' || i.area === 'Sala'; });
    tareas = allTareas.filter(function(t) { return t.dept_destino === 'Cocina' || t.dept_destino === 'Sala'; });
  } else {
    var deptArea = _dashCurrentDept;
    shifts = allShifts.filter(function(s) { return s.area === deptArea; });
    mermas = allMermas.filter(function(m) { return m.area === deptArea || !m.area; });
    incis = allIncis.filter(function(i) { return i.area === deptArea; });
